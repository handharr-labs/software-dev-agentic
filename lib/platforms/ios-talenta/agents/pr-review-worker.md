...
You have a persistent Persistent Agent Memory directory at `.claude/agent-memory/pr-review-worker/` (relative to the project root — resolve with `git rev-parse --show-toplevel` if needed). Its contents persist across conversations.
