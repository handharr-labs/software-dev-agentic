...
description: Create BLoC tests for Flutter Qontak using bloc_test. Checks .status.isHasData/.status.isError via predicate<State>, named constructor params in setUp.
user-invocable: false
---

Create BLoC tests following `lib/platforms/flutter-qontak-chat/reference/code-architecture/testing-impl.md ## BLoC Tests section`.

## Steps

1. **Read** the BLoC's Event, State, and BLoC files completely — map all events, state fields, and use case constructor params
2. **Update** `test/helpers/mocks/[feature]_mocks.dart` — add `MockSpec` for each UseCase the BLoC receives
...
import 'package:[prefix]_[feature]/src/presentation/blocs/[feature]_bloc.dart';

import '../../helpers/mocks/[feature]_mocks.mocks.dart';
import '../../helpers/fixtures/[feature]_fixtures.dart';

void main() {
  late MockGet[Concept] mockGet[Concept];
  late [Feature]Bloc bloc;

  setUp(() {
    mockGet[Concept] = MockGet[Concept]();
    bloc = [Feature]Bloc(get[Concept]UseCase: mockGet[Concept]); // named param
  });
  ...
  group('[Feature]Bloc', () {
    ...
    group('[Verb][Concept]', () {
      blocTest<[Feature]Bloc, [Feature]State>(
        ...
        setUp: () {
          when(mockGet[Concept].call(any))
              .thenAnswer((_) async => Right(t[Concept]));
        },
        build: () => bloc,
        act: (b) => b.add([Feature]Event.[verb][Concept]()),
        expect: () => [
        ...
        verify: (_) => verify(mockGet[Concept].call(any)).called(1),
      );
...
- BLoC constructor uses **named params** — `[Feature]Bloc(get[Concept]UseCase: mockGet[Concept])`
