...
- design-system catalogs: `discipline: design` + `tags: [design-system]` (there is no `area` facet)
...
4. **artifact** — from the draft's H1/title, kebab-cased; reuse an existing artifact name when the topic already exists. For a design-system catalog, set `discipline: design` + `tags: [design-system]`.
5. **sections** — ensure the body uses `##` per concept. If the draft is one blob, propose a `##` split and show it. Never discard preamble — fold it into the first section or an `## Overview`.

## Canonical Path (3-level — no `area` segment)

- universal: `{knowledge_root}/universal/{discipline}/{artifact}.md`
- platform:  `{knowledge_root}/platform/{platform}/{discipline}/{artifact}.md`
- project:   `{knowledge_root}/projects/{project}/{discipline}/{artifact}.md`
