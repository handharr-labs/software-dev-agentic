...
| Area | [KMS](kms/kms-glossary.md) | *Removed 2026-07-03* — was a `core`/`design-system` path segment; design-system catalogs now use `discipline: design` + `tags: [design-system]`. |
| Layer | [KMS](kms/kms-glossary.md) | CLEAN facet `domain`/`data`/`presentation`/`cross` for per-agent scoping; derived frontmatter → `#`-topic marker → `cross`. |
| Owner | [KMS](kms/kms-glossary.md) | Lifecycle facet — `curated` (hand-owned) or `extracted` (scanner-regenerated). |
...
| Knowledge Path | [KMS](kms/kms-glossary.md) | The ordered tuple of Rosetta Stone terms (`scope` → `platform`/`project` → `discipline` → `artifact` → `topic` → `section`) that addresses a knowledge node. |
| Knowledge Path Structure | [KMS](kms/kms-glossary.md) | The directory + heading convention every Knowledge Path is an instance of — `{scope}/[{platform}\|{project}]/{discipline}/{artifact}.md` (3-level) plus `#`→topic / `##`→one node chunking. |
...
| Scoping funnel | [KMS](kms/kms-glossary.md) | `kms_list` narrowing order: `platform`/`project` → `discipline` → `layer` → `artifact` → `topic` → `section`. |
