...
### Phase 4 — Stable Architecture

| Task | Status |
|---|---|
| Define `KnowledgeSource` abstract interface — `read() -> Iterator[KnowledgeNode]` | ✅ Done — `kms/domain/sources/base.py` |
| Implement `MarkdownSource` adapter — wraps current `lib/core/knowledge/*.md` reader | ✅ Done — `kms/domain/sources/markdown.py` |
| Implement `CodebaseSource` adapter — wraps scan agent output | ✅ Done — `kms/domain/sources/codebase.py` (stub; scan agent writes via `kms_upsert` directly) |
| Implement `ConfluenceSource` adapter | ✅ Done — `kms/domain/sources/confluence.py` (stub; full reader pending auth + parser) |
| Unified seed runner — reads `kms/sources.yaml`, source-agnostic, per-source isolation | ✅ Done — `kms/scripts/seed_kms.py` rewritten |
| `kms/sources.yaml` — source registry (name, type, path/url, owns, last_seeded) | ✅ Done — initial entries: flutter-base, flutter-mobile-talenta, ios-talenta, web |
| Seed runner: skip-on-unavailable — inaccessible source skipped, existing nodes preserved | ✅ Decided |
| Seed runner: section ownership — each source only upserts its owned sections, never overwrites others | ✅ Decided |
| Seed runner: `--source <name>` and `--type <type>` flags — re-seed one source or one type only | ✅ Decided |
| Seed runner: `--add <path\|url>` — auto-detect type, seed, and register in `sources.yaml` in one step | ✅ Decided |
| Seed runner: type detection rules — `pubspec.yaml`→flutter, `package.json`→web, `*.xcodeproj`→ios, confluence URL, GitHub URL | ✅ Decided |
| Seed runner: `--seed` flag on `build-plugin.sh` — off by default, explicit opt-in | ✅ Decided |
| Lock metadata schema — mandatory vs optional fields, allowed values, `schema_version` | ✅ Decided — see Knowledge Schema section |
| Add `scope` field — `universal \| platform \| project`; replaces null-inference on platform+project | ✅ Decided |
| Expand `discipline` vocabulary — add `architecture`, `agile` for full SDLC coverage | ✅ Decided — 9 values locked |
| `pattern` naming — keep as neutral term by convention across all disciplines | ✅ Decided |
| Collection strategy — single collection (locked); cascade enforced via metadata not collection separation | ✅ Decided |
| `kms/domain/schema.py` — constants file: SCOPE_VALUES, DISCIPLINE_VALUES, MANDATORY_FIELDS, SCHEMA_VERSION, etc. | ✅ Done — `kms/domain/schema.py` |
| Incremental seeding — change detection via content hash (`content_hash` metadata field); uniform across all source types | ✅ Done — implemented in `seed_kms.py` + `chroma_repository.py` |
| `owns` enforcement — in `UpsertKnowledge` use case only; strips non-owned sections before merge, adapters have no enforcement burden | ✅ Done — `kms/domain/use_cases/upsert_knowledge.py` |
| Name derivation for `--add` — detect worker proposes name from path/url (dir name, page title, repo name); confirms full entry with user once before writing | ✅ Done — `kms-source-detect-worker` + `seed_kms.py --add` |
| Update `seed_kms.py` to use unified runner + source adapters | ✅ Done — full rewrite with `--source`, `--type`, `--add` flags |
| Document source adapter contract in `kms/README.md` | ✅ Done |
| Scaffold `/kms-seed` skill — user-invocable entry point | ✅ Done — `.claude/skills/kms-seed/SKILL.md` |
| Scaffold `kms-seed-orchestrator` — reads sources.yaml, filters by flags, spawns workers | ✅ Done — `.claude/agents/kms-seed-orchestrator.md` |
| Scaffold `kms-source-detect-worker` — detects type + proposes sources.yaml entry for `--add` flow | ✅ Done — `.claude/agents/kms-source-detect-worker.md` |
| Scaffold `kms-seed-worker` — handles one source: accessibility check, seed, update last_seeded | ✅ Done — `.claude/agents/kms-seed-worker.md` |
...
**Source of truth:** ChromaDB is authoritative. Knowledge enrichment happens in this repo — engineers add sources here, run the seed runner, and the plugin bundles the result.

**Seeding architecture (locked):**

Sources are registered in `kms/sources.yaml`. Each source declares its type, location, owned sections, and last seeded timestamp. The seed runner reads this manifest — it never hardcodes sources.
...
sources:
  - name: flutter-base-knowledge
    type: markdown
    path: lib/core/knowledge/flutter
    owns: [theory, definition]
    last_seeded: 2026-06-04

  - name: flutter-mobile-talenta
    type: codebase
    path: /path/to/flutter-mobile-talenta   # local clone required
    owns: [code_pattern, source_file]
    last_seeded: null

  - name: clean-architecture-confluence
    type: confluence
    url: https://confluence.mekari.com/pages/12345
    owns: [theory, rationale]
    last_seeded: null
```

**Seed runner behavior (locked):**
- **Skip-on-unavailable** — if a source path/url is inaccessible, skip it with a warning. Never abort. Never remove or overwrite existing nodes from that source.
- **Section ownership** — each source only upserts its declared `owns` sections. Other sections on the same node are never touched.
- **Incremental** — compares file `updated_at` vs ChromaDB `updated_at`; only re-seeds changed nodes.
- **Selective flags** — `--source <name>` re-seeds one source; `--type <type>` re-seeds all sources of a type.
- **Auto-register** — `--add <path|url>` detects source type, seeds it, and appends to `sources.yaml` in one step. If already registered, updates `last_seeded` only.

**Type detection for `--add` (locked):**

| Input | Detected type | Default `owns` |
|---|---|---|
| Local path with `*.md` files | `markdown` | `[theory, definition]` |
| Local path with `pubspec.yaml` | `codebase` (flutter) | `[code_pattern, source_file]` |
| Local path with `package.json` | `codebase` (web) | `[code_pattern, source_file]` |
| Local path with `*.xcodeproj` | `codebase` (ios) | `[code_pattern, source_file]` |
| URL matching `confluence.` | `confluence` | `[theory, rationale]` |
| GitHub URL | `codebase` (remote) | `[code_pattern, source_file]` |

Engineer can override `owns` in `sources.yaml` after registration.
...
python -m kms.scripts.seed_kms --source flutter-base-knowledge

# Seed all sources of a type
python -m kms.scripts.seed_kms --type markdown

# Detect, seed, and register a new source in one step
python -m kms.scripts.seed_kms --add /path/to/flutter-mobile-talenta
python -m kms.scripts.seed_kms --add https://confluence.mekari.com/pages/12345

# Build plugin without re-seeding (default)
build-plugin.sh --platform=flutter

# Build plugin and re-seed before bundling
build-plugin.sh --platform=flutter --seed
```
...
```
scope → platform → project → discipline → topic → pattern
```
...
universal                    -- WHAT: general principles (Clean Architecture, SOLID, SDLC-wide)
  └─ platform (flutter)      -- HOW: implemented in this platform (BLoC, get_it, layer structure)
       └─ project (talenta)  -- WHERE: deviations for this project only (specific constraints, exceptions)
```

Query resolution order: `project → platform → universal`. Returns first match. A project node is only created when a real deviation exists — most knowledge lives at platform or universal tier.

**Collection strategy (locked): single collection.** The cascade lives in `scope + platform + project` metadata fields — not in collection separation. Splitting by platform would break cascade fallthrough (project → platform → universal) which requires all tiers queryable in one call.

**Discipline scope pattern (locked):**

| Discipline | Natural scope | Rationale |
|---|---|---|
| `engineering` | platform | Implementation is always platform-specific |
| `design` | universal → platform | Components are universal; platform rendering varies |
| `qa` | universal | Test strategy is platform-agnostic |
| `devops` | universal → platform | CI/CD is mostly universal; some platform tooling differs |
| `security` | universal | Threat models and mitigations are platform-agnostic |
| `code_review` | universal → platform | Rules are universal; code examples are platform-specific |
| `product` | project | PRDs and decisions are always project-specific |
| `architecture` | universal → platform | Principles are universal; ADRs may be platform or project scoped |
| `agile` | universal | Ceremonies and rituals are team-wide, not platform-specific |

**Hierarchy examples:**

```
-- Engineering: same topic+pattern at all three tiers
scope=universal,  platform=null,    project=null     → engineering, domain, use_case  (WHAT: principle)
scope=platform,   platform=flutter, project=null     → engineering, domain, use_case  (HOW: flutter impl)
scope=project,    platform=flutter, project=talenta  → engineering, domain, use_case  (WHERE: talenta deviation)

-- Engineering: platform-scoped cross-cutting concerns
scope=platform, flutter, null, engineering, state_management,     bloc
scope=platform, flutter, null, engineering, dependency_injection, get_it
scope=platform, flutter, null, engineering, navigation,           go_router

-- Universal disciplines (scope=universal, platform=null, project=null)
universal, agile,        sprint_ceremony,  retrospective
universal, architecture, clean_arch,       dependency_rule
universal, qa,           unit_testing,     mock_setup
universal, security,     auth_patterns,    jwt_handling

-- Project-scoped product knowledge
scope=project, flutter, talenta, product, onboarding_flow, acceptance_criteria
```

**Discipline vocabulary (locked — SDLC-scale):**
`engineering` · `design` · `qa` · `devops` · `security` · `code_review` · `product` · `architecture` · `agile`

| Discipline | Role / Work Area |
|---|---|
| `engineering` | Software engineers — layers, patterns, code |
| `design` | Designers — UX/UI components, guidelines |
| `qa` | QA engineers — test strategy, checklists, templates |
| `devops` | Platform/DevOps — CI/CD, infra, runbooks |
| `security` | Security engineers — threat models, mitigations |
| `code_review` | All engineers — review rules, PR standards |
| `product` | Product managers — PRDs, decisions, acceptance criteria |
| `architecture` | Tech leads/Architects — ADRs, system design, tech strategy |
| `agile` | Scrum masters/Teams — ceremonies, retrospectives, sprint rituals |
...
- `scope` field added — explicit cascade intent (`universal | platform | project`). Replaces null-inference on `platform` + `project`.
- `pattern` kept as neutral term across all disciplines — for engineering it's a code pattern, for qa it's a checklist type, for agile it's a ceremony template. Convention, not type.
- `discipline` vocabulary locked to 9 values. Expand by updating `kms/domain/schema.py` + this doc together.
- `schema_version` field added — increment on any breaking field change so seed scripts and agents can detect mismatches.
- `content_hash` field added — SHA hash of full document content; used by seed runner for incremental change detection across all source types uniformly.
- All vocabulary constants live in `kms/domain/schema.py` — single source of truth for seed runner, adapters, use cases, and agents.
- `owns` enforcement lives in `UpsertKnowledge` use case — strips non-owned sections before merge; adapters have no enforcement burden.
...
collection.add(
  ...
  documents=["## Theory\n...\n## Definition\n...\n## Code Pattern\n..."],
  metadatas=[{
    "scope":        "platform",        # universal | platform | project
    "platform":     "flutter",         # flutter | web | ios | android — omit if scope=universal
    "project":      None,              # talenta | jurnal | qontak-crm — omit if scope != project
    "discipline":   "engineering",     # see DISCIPLINE_VALUES in kms/domain/schema.py
    "topic":        "domain",          # free string — process area or architecture layer
    "pattern":      "use_case",        # free string — neutral term across all disciplines
    "tags":         "[]",              # JSON array string
    "source_file":  "lib/core/knowledge/flutter/engineering/domain/use_case.md",
    "updated_at":   "2026-06-03",
    "content_hash": "a3f1c2...",       # SHA hash of document body — used for incremental seed detection
...
## Seeding Workflow — Agentic Layer

The seed runner (`seed_kms.py`) is a Python script. The workflow around it — source detection, registry updates, failure handling, and user confirmation — is handled by agents and skills.

**Components (locked):**

| Component | Type | Responsibility |
|---|---|---|
| `/kms-seed` | skill | User-invocable entry point — passes args to orchestrator |
| `kms-seed-orchestrator` | orchestrator agent | Reads `sources.yaml`, applies flag filters, spawns one `kms-seed-worker` per source, reports summary |
| `kms-source-detect-worker` | worker agent | `--add` flow only — detects type from path/url, proposes `sources.yaml` entry, confirms with user before registering |
| `kms-seed-worker` | worker agent | Handles one source — accessibility check, calls `seed_kms.py`, updates `last_seeded` in `sources.yaml`, returns result |

**Flow — seed all available:**
```
/kms-seed
  → kms-seed-orchestrator
       ├─ reads sources.yaml, no filter
       ├─ spawns kms-seed-worker per source (parallel)
       │     ├─ accessible → seed → update last_seeded
       │     └─ inaccessible → skip, preserve existing nodes
       └─ reports: N seeded, M skipped
```

**Flow — add and register a new source:**
```
/kms-seed --add /path/to/flutter-mobile-talenta
  → kms-seed-orchestrator
       → kms-source-detect-worker
            detects: type=codebase (flutter), owns=[code_pattern, source_file]
            derives name from directory name → "flutter-mobile-talenta"
            presents full proposed entry to user (name, type, owns) — one confirmation
            user accepts or overrides
       → kms-seed-worker
            seeds source, appends confirmed entry to sources.yaml, updates last_seeded
       reports: 47 nodes seeded, sources.yaml updated
```

**Flow — seed one source:**
```
/kms-seed --source flutter-base-knowledge
  → kms-seed-orchestrator
       → kms-seed-worker (single source only)
```
...
Define a source-agnostic architecture so adding a new platform or knowledge source type requires no changes to the seed runner or domain layer.
...
**Source adapters (each implements `KnowledgeSource`):**
- `MarkdownSource(root: Path)` — current behavior, reads `lib/core/knowledge/*.md`
- `CodebaseSource(scan_output: Path)` — wraps scan agent JSON output
- Future: `ConfluenceSource`, `MetricsSource`, `BestPracticeSource`, etc.

**Unified seed runner:**
```python
def seed(sources: list[KnowledgeSource], db_path: str) -> None:
    repo = ChromaKnowledgeRepository(db_path)
    upsert = UpsertKnowledge(repo)
    for source in sources:
        for node in source.read():
            upsert.execute(node)
```

**Metadata schema (locked contract):**

| Field | Mandatory | Allowed values |
|---|---|---|
| `discipline` | ✅ | `engineering`, `design`, `qa`, `devops`, `security`, `code_review`, `product` |
| `topic` | ✅ | free string |
| `pattern` | ✅ | free string |
| `platform` | ⬜ | `flutter`, `ios`, `android`, `web`, `null` |
| `project` | ⬜ | `talenta`, `jurnal`, `qontak-crm`, `qontak-chat`, `null` |
| `tags` | ⬜ | JSON array string |
| `source_file` | ⬜ | absolute path |
| `updated_at` | ⬜ | ISO date string |
| `schema_version` | ✅ | `"1"` (increment on breaking changes) |

**Collection strategy decision:** single collection vs per-platform. Must be decided before Phase 5 — breaking change to reverse.

- [ ] Define `KnowledgeSource` abstract interface
- [ ] Implement `MarkdownSource` adapter
- [ ] Implement `CodebaseSource` adapter
- [ ] Unified seed runner — source-agnostic
- [ ] Lock metadata schema + add `schema_version` field
- [ ] Decide and document collection strategy
- [ ] Update `seed_kms.py` to use unified runner
- [ ] Document source adapter contract in `kms/README.md`

### Phase 5 — Extraction (when needed)
