...
4. **Cutover + reconcile the docs** — ✅ **DONE** — live `knowledge` collection reseeded with the new schema; MCP needs no change (collection name unchanged). Reconciled `kms-knowledge-source-rules.md` + `kms-design-principles.md` and flagged `kms-knowledge-restructure-initiative.md` as superseded. `sources.yaml` path fixed.

4b. **3-level restructure + `area` removal** — ✅ **DONE (breaking)** — moved every file up one level (dropped `core/`/`design-system/`; design-system → `tags: [design-system]` frontmatter), flattened the seeder traversal, and removed `area` from the entire stack **including the distributed MCP tool signatures** (`kms_fetch`/`kms_list`/`kms_query`/`kms_upsert`) — `schema_version` → `4`. Reseeded (836 nodes, gate re-passed: recall@5=0.90, MRR=0.695 — identical, confirming a purely structural change). Reconciled the deeper reference docs (`kms-conventions.md`, `kms-glossary.md`, `kms-directory-structure.md`, root `glossary.md`) and the `kms-classify-worker`. Still open: `kms-source-audit-worker` still validates against old path rules.
5. **Agent scoping** — ✅ **DONE** — additive optional `layer` on `kms_query`/`kms_list` (server auto-unions `cross` → `layer ∈ {layer, cross}`), threaded through `ListKnowledge` + repository; `aegis-kms-load` forwards a `layer` input; planners pass theirs (domain/data/presentation), `app-planner` omits it (cross-layer wiring). Verified disjoint scoping, zero leakage. Non-breaking (new optional params only).
6. **Contribution flow** — ✅ **DONE** — `knowledge-sources/_inbox/` on-ramp (sibling of the seeded tiers → never seeded directly), `kms-classify-worker` (infers facets from a loose draft → writes a frontmatter-carrying canonical file or `.proposed.md` merge; Transparent-Steering report), `kms-contribute-orchestrator` + `/kms-contribute` skill (classify-only; never seeds — git/`/kms-seed` review gate preserved). Verified `_inbox` is ignored by the seeder and placed files carry `layer`/`owner`.
