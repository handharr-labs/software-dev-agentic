...
4. **Cutover + reconcile the docs** — ✅ **DONE** — live `knowledge` collection reseeded with the new schema (836 nodes, gate re-passed on it: recall@5=0.90, MRR=0.695); scratch `knowledge_v2` dropped; MCP needs no change (collection name unchanged). Reconciled `kms-knowledge-source-rules.md` + `kms-design-principles.md` (frontmatter-authoritative, `##`-concept chunking, `layer`/`owner`) and flagged `kms-knowledge-restructure-initiative.md` as superseded. `sources.yaml` path fixed in step 4-prep. **Still TODO (split out, breaking):** `area` removal + 3-level path move + `kms-source-audit-worker` update — deferred because dropping `area` changes the distributed MCP tool signatures (`kms_fetch`/`kms_list`/`kms_query`/`kms_upsert`) and requires a file-tree migration. Tracked as a follow-up.
