...
9. **Transition to implementation** — route to `developer-build-feature` — see Transition section
...
**The terminal state is routing to `developer-build-feature`.** Do NOT write code or scaffold before the user confirms the execution path.
...
Once the user approves the spec, invoke the `developer-build-feature` skill via the Skill tool, passing `<spec-path>` as the argument. Do not ask the user to run it manually.
